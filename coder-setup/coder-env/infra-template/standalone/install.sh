...
# S'assurer que ~/.local/bin est dans le PATH (mise s'y installe par défaut)
case ":${PATH}:" in
  *":${HOME}/.local/bin:"*) ;;
...
if command -v mise &>/dev/null; then
  ...
  case "${OS}" in
    Linux)
      ...
      # mise s'installe dans ~/.local/bin qui n'est pas forcément dans le PATH
      # du shell courant => on l'ajoute pour la suite du script.
      case ":${PATH}:" in
        *":${HOME}/.local/bin:"*) ;;
        *) export PATH="${HOME}/.local/bin:${PATH}" ;;
      esac
...
# On en profite pour ajouter ~/.local/bin au PATH (mise s'y installe par défaut
# mais ce répertoire n'est pas toujours dans le PATH interactif).
...
echo ""
echo "  Or manually:"
echo "    cd <project-root>/infra/live/001_k3d_init_cluster"
