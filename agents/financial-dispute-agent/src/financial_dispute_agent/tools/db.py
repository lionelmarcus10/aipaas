...
Three modes:
  1. NFS (k3d / EKS): DB_LOCAL_PATH env var points to the NFS-mounted file.
     The initContainer generates the DB via setup_db.py on first boot.
  2. AWS Lambda: DB is downloaded from S3 to /tmp at cold start
     (env vars: DB_S3_BUCKET, DB_S3_KEY — set by Terraform).
  3. Local dev: DB is at data/financial_dispute.duckdb relative to the repo.
"""
...
LOCAL_DB_PATH = Path(__file__).parent.parent.parent.parent / "data" / "financial_dispute.duckdb"

# Lambda /tmp path (downloaded from S3 at cold start)
LAMBDA_DB_PATH = Path("/tmp/financial_dispute.duckdb")
...
def _download_from_s3() -> Path:
    ...
    key = os.environ.get("DB_S3_KEY", "financial_dispute.duckdb")
...
def get_db_path() -> Path:
    ...
    Priority:
      1. DB_LOCAL_PATH env var (k3d/EKS NFS mount) — explicit path to the DB file
      2. DB_S3_BUCKET env var (Lambda) — download from S3 to /tmp
      3. LOCAL_DB_PATH (local dev) — relative to the repo
    """
    # 1. NFS mount (k3d / EKS) — the initContainer generated the DB on the NFS share
    if os.environ.get("DB_LOCAL_PATH"):
        return Path(os.environ["DB_LOCAL_PATH"])

    # 2. Lambda — download from S3
    if os.environ.get("DB_S3_BUCKET"):
        return _download_from_s3()

    # 3. Local dev — use the file relative to the repo
    return LOCAL_DB_PATH


def get_connection() -> duckdb.DuckDBPyConnection:
    """Return a read-only DuckDB connection to the financial dispute database."""
    db_path = get_db_path()
